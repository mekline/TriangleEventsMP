function bioloc2(subjID,designOrder,runnum,randSeed)

% Bioloc2: only has 2 conditions (bio motion, SFM) - no scrambles.
...
%{
% Design for bioloc2:
design = [0 1 2 1 2 1 2 0 2 1 2 1 2 1 0;
          0 2 1 2 1 2 1 0 1 2 1 2 1 2 0;
          0 1 2 1 2 1 2 0 2 1 2 1 2 1 0;
          0 2 1 2 1 2 1 0 1 2 1 2 1 2 0];
...
%}
    ...
    [subjID, designID] = deal(answer{:});
    %}
    designID = designOrder(runnum);
    ...
    % filename
    % fname = sprintf(fullfile('%s', '%s_s%s_r%d.txt'), file_directory, file_prefix, subjID, designID);
...
%             end;
        ...
        if condition == 0
            ...
            while GetSecs < t_end
                ...
                if ~preloading && block+1 <= run_size && run(block+1) ~= 0
                    ...
                    %disp(next_moviename);
                    Screen('OpenMovie', wPtr, next_moviename, 1);   % NOTE: this line sometimes fails inscrutably.
            ...
            if files_no > 0
                ...
                for movie=1:movies_no
                    ...
                    anim = (movieblock-1)*(movs_per_dir+repeats) + movie;
                    try
                        buttonpress = 0;
                        t_mov_start = GetSecs;
                        ...
                        while GetSecs < t_end
                            ...
                            %if keyIsDown==1, disp('BUTTON'); end;
                            if (keyIsDown==1 && ~keyCode(KbName('=+')) && ~keyCode(KbName('+')) && ~keyCode(KbName('=')))
                                ...
                                %disp(['BUTTON!! -- ' int2str(buttonpress)])
                            end;
                            
                            Screen('Close', tex);
                        end
                        
                        if iscell(buttonpress), buttonpress = 0; end;
    ...
    endtime_actual = t_exp_end-t_exp_start;
    exptduration = (sum(run~=0)*(movs_per_dir+repeats)*(blank_time))+mov_length+fixation_time*sum(run==0);
    fprintf('\n\nrun: %5.5f, calc: %5.5f => time-loss: %5.5f \n', endtime_actual, exptduration, endtime_actual-exptduration);
    ...
    for r = 1:length(run)
        if run(r)==0
            ...
            durs(r) = (movs_per_dir+repeats)*(blank_time+2);
        end;
        if r==1
            onsets(r)=0;
        else
    ...
    for c=1:length(condNames)
        ...
        spm_inputs(c).ons  = (onsets(realrun == c)'/2)+1;
        spm_inputs(c).dur  = durs(realrun == c)'/2;
    
        fsl_inputs(c).name = condNames{c};
        fsl_inputs(c).regressor = [onsets(realrun == c)' durs(realrun == c)' ones(length(onsets(realrun == c)),1)];
    end;
    
    con_info(1).name = 'Bio > Sfm';
    con_info(1).vals = [1 -1];
    con_info(2).name = 'Sfm > Bio';
    con_info(2).vals = [-1 1];
    con_info(3).name = 'Bio > Rest';
    con_info(3).vals = [1 0];
    con_info(4).name = 'Sfm > Rest';
    con_info(4).vals = [0 1];
    con_info(5).name = 'All > Rest';
    con_info(5).vals = [1 1];
    con_info(6).name = 'Rest > All';
    con_info(6).vals = [-1 -1];
    
    
    behavfile = [behavpath '/' subjID '.bioloc2.' int2str(runnum) '.mat'];
    while ~overwrite
    ...
    save(behavfile,'subjID','designID','designOrder','runnum','design','spm_inputs','fsl_inputs','con_info','endtime_actual','exptduration','accuracy','stat_hit','stat_miss','stat_fa','stat_cr','rbutton','rt','movienames');
